...
export class ChatComponent implements OnInit {
    ...
    if (this.text.replace(/\s/g, '').length) {
      ...
      if (this.text.startsWith('/')) {
        ...
        this.response = this.http.post<[{
          text: string
        }, { image: string }]>('https://vm07.htl-leonding.ac.at:4200/core/webhooks/rest/webhook',
  ...
  sendMessageButton(text, payload): void {
    if (text.replace(/\s/g, '').length) {
      ...
      // this.chatService.sendMessage(this.text);
      this.response = this.http.post<any>('https://vm07.htl-leonding.ac.at:4200/core/webhooks/rest/webhook',
